...
#include "common/odp_perf_common.h"
...
static void print_usage(const dynamic_defs_t *dyn_defs)
{
	...
	printf("\n"
	       ...
	       "  --wait_link <sec>  Wait up to <sec> seconds for each network link to be up.\n"
...
static odp_bool_t setup_pktios(prog_config_t *config)
{
	...
	uint32_t wait_sec = config->wait_sec;
	odp_pktio_t handle;
	char *name;
	int ret;
	uint8_t i;
	...
	/* Wait for links to be up and print info */
	for (i = 0; i < num_ifs; i++) {
		handle = prog_conf->pktios[i].handle;
		name = config->pktios[i].name;

		if (wait_sec && check_link_status_wait(handle, name, wait_sec))
			return false;

		print_link_info(handle, name);
	}
