...
static void test_common_extract_options(int argc, char *argv[])
{
	int i;
	size_t ret = 0;
	size_t current_len = strlen(gbl_data.args);
	size_t remaining_space = MAX_ARGS_LEN - current_len;

	if (argc > 1)
		ret = snprintf(gbl_data.args + current_len, remaining_space, "%s", argv[1]);

	if (ret >= remaining_space)
		goto exit;

	for (i = 2; i < argc; i++) {
		current_len = strlen(gbl_data.args);
		remaining_space = MAX_ARGS_LEN - current_len;
		ret = snprintf(gbl_data.args + current_len, remaining_space, " %s", argv[i]);
		if (ret >= remaining_space)
			goto exit;
	}
...
exit:
	ODPH_ERR("More options and arguments than args[MAX_ARGS_LEN] (%i) can fit\n",
		 MAX_ARGS_LEN);
}
...
int test_common_parse_options(int argc, char *argv[])
{
	...
	if (gbl_data.common_options.is_export) {
		...
		test_common_extract_options(argc, argv);
	}
...
int test_common_options(test_common_options_t *options)
{
	...
	fprintf(gbl_data.file, "#%s;%s\n", gbl_data.filename, gbl_data.args);
